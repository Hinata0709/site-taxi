...
function initTestimonialsCarousel() {
    ...
    function getCardsPerView() {
        ...
        if (viewportWidth < 768) return 1.5;
    ...
    function createDots() {
        ...
        const totalDots = Math.ceil(cards.length / Math.floor(cardsPerView));

        for (let i = 0; i < totalDots; i++) {
            const dot = document.createElement('button');
            dot.classList.add('carousel-dot');
            if (i === 0) dot.classList.add('active');
            dot.addEventListener('click', () => goToSlide(i * Math.floor(cardsPerView)));
    ...
    function updateDots() {
        ...
        const activeDot = Math.floor(currentIndex / Math.floor(cardsPerView));
    ...
    window.addEventListener('resize', () => {
        ...
        maxIndex = Math.max(0, cards.length - Math.floor(cardsPerView));
        createDots();
        if (currentIndex > maxIndex) {
            goToSlide(maxIndex);
        } else {
...
/* Console Welcome Message
   ===================================================== */
console.log(
    '%c ID TAXI %c Service de taxi professionnel ',
    'background: #F7B731; color: #0f172a; font-weight: bold; padding: 5px 10px; border-radius: 5px 0 0 5px;',
    'background: #0f172a; color: #F7B731; padding: 5px 10px; border-radius: 0 5px 5px 0;'
);
console.log('Site web ID Taxi - Saint-Gratien, Val-d\'Oise');
console.log('Contact: 06 85 73 03 39 | reservation@id-taxi.fr');
